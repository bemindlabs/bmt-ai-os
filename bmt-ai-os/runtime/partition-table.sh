...
if [[ "$DRY_RUN" == "false" ]]; then
    ...
    mkdir -p \
        ...
        "$TMPDIR_MOUNT/bmt-ai-os/db" \
        "$TMPDIR_MOUNT/bmt-ai-os/rag/ingest-queue" \
        "$TMPDIR_MOUNT/bmt-ai-os/cache" \
        "$TMPDIR_MOUNT/secrets/tls" \
        "$TMPDIR_MOUNT/logs/bmt-ai-os"
    ...
    run mkdir -p /data/bmt-ai-os/db
    run mkdir -p /data/bmt-ai-os/rag/ingest-queue
    run mkdir -p /data/bmt-ai-os/cache
    run mkdir -p /data/secrets/tls
    run mkdir -p /data/logs/bmt-ai-os
